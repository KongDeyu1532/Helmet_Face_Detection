...
'''
def FaceRecognitionProcess(image, q , startEvent, endEvnet):
    while (True):
        ...
        # 模拟：
        startEvent.wait()               # 等待主进程准备好图片后，置位Event对象
        ...
        if(index >=0 and index < 5):
            ...
            q.put(FaceRecognitionResult(isSuccess, objectName))
            print("\t检测到的信息：", '识别成功：',  objectName)
        else:
            q.put(FaceRecognitionResult(False, ''))

        startEvent.clear()      # 清除标志，由主进程重置，以便开始下一帧图像的处理
        endEvnet.set()          # 重置事件，向主进程报告本次处理结束

# 头盔检测,image 检测的图像，q--保存检测结果
def HelmetDetectionProcess(image , q, startEvent, endEvnet):
    while(True):
        ...
        if result == 0:
            ...
            q.put(HelmetDetectionResult(hasHelmet, x, y))
        else:
            q.put(HelmetDetectionResult(False, -1, -1))
...
if(__name__=='__main__'):
    ...
    faceRecognition = multiprocessing.Process(target=FaceRecognitionProcess,
                                              args=(None , faceQueue, faceStartEvent,faceEndEvent))
    helmetDetection = multiprocessing.Process(target=HelmetDetectionProcess,
                                              args=(None , helmetQueue, helmetStartEvent, helmetEndEvent))
    # 置进程是否为守护进程，如果是守护进程，则主语种结束时，子进程立即结束
    faceRecognition.daemon = True
    helmetDetection.daemon = True
