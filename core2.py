...
class CoreUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.faceTrackerCheckBox.stateChanged.connect(
            lambda: self.enableFaceTracker())

        self.faceRecognizerCheckBox.stateChanged.connect(
            lambda: self.enableFaceRecognizer())


        self.panalarmCheckBox.stateChanged.connect(lambda: self.enablePanalarm())

        # 直方图均衡化
        self.equalizeHistCheckBox.stateChanged.connect(
            lambda: self.enableEqualizeHist())

        # 调试模式
        self.debugCheckBox.stateChanged.connect(lambda: self.enableDebug())
        self.confidenceThresholdSlider.valueChanged.connect(
            lambda: self.setConfidenceThreshold())
        self.autoAlarmThresholdSlider.valueChanged.connect(
            lambda: self.setAutoAlarmThreshold())
        ...
        # 是否开启人脸跟踪
    def enableFaceTracker(self):
            if self.faceTrackerCheckBox.isChecked():
                self.isFaceTrackerEnabled = True
                self.statusBar().showMessage('人脸跟踪：开启')
            else:
                self.isFaceTrackerEnabled = False
                self.statusBar().showMessage('人脸跟踪：关闭')

        # 是否开启人脸识别
    def enableFaceRecognizer(self):
            if self.faceRecognizerCheckBox.isChecked():
                if self.isFaceTrackerEnabled:
                    self.isFaceRecognizerEnabled = True
                    self.statusBar().showMessage('人脸识别：开启')
                else:
                    self.logQueue.put('Error：操作失败，请先开启人脸跟踪')
                    self.faceRecognizerCheckBox.setCheckState(Qt.Unchecked)
                    self.faceRecognizerCheckBox.setChecked(False)
            else:
                self.isFaceRecognizerEnabled = False
                self.statusBar().showMessage('人脸识别：关闭')

        # 是否开启报警系统
    def enablePanalarm(self):
            if self.panalarmCheckBox.isChecked():
                self.isPanalarmEnabled = True
                self.statusBar().showMessage('报警系统：开启')
            else:
                self.isPanalarmEnabled = False
                self.statusBar().showMessage('报警系统：关闭')

        # 是否开启调试模式
    def enableDebug(self):
            if self.debugCheckBox.isChecked():
                self.isDebugMode = True
                self.statusBar().showMessage('调试模式：开启')
            else:
                self.isDebugMode = False
                self.statusBar().showMessage('调试模式：关闭')

        # 设置置信度阈值
    def setConfidenceThreshold(self):
            if self.isDebugMode:
                self.confidenceThreshold = self.confidenceThresholdSlider.value()
                self.statusBar().showMessage('置信度阈值：{}'.format(self.confidenceThreshold))

        # 设置自动报警阈值
    def setAutoAlarmThreshold(self):
            if self.isDebugMode:
                self.autoAlarmThreshold = self.autoAlarmThresholdSlider.value()
                self.statusBar().showMessage('自动报警阈值：{}'.format(self.autoAlarmThreshold))

        # 直方图均衡化
    def enableEqualizeHist(self):
            if self.equalizeHistCheckBox.isChecked():
                self.isEqualizeHistEnabled = True
                self.statusBar().showMessage('直方图均衡化：开启')
            else:
                self.isEqualizeHistEnabled = False
                self.statusBar().showMessage('直方图均衡化：关闭')
    ...
    def detectFace(self,frame):
        ...
        #faces的返回值类型：如果有人脸则返回ndarray类型，没有则返回tuple()。
    ...
    def MyCore(self):
        ...
        FaceRecognition = faceRcognition.FaceProcessingThread(faceQueue, faceStartEvent, faceEndEvent)
        HelmetDetection = helmetDetect.HelmetProcessingThread(helmetQueue, helmetStartEvent, helmetEndEvent)
        ...
        while (True):
            if self.cap.isOpened():
                ...
                yes = isinstance(faces,ndarray)
                if (yes):
                    faceStartEvent.set()
                    for(x,y,w,h) in faces:
                        #把照片参数传入到队列里
                        _in=inputFaceQueue(x,y,w,h,frame)
                        faceQueue.put(_in)
                        print("开始识别图像中的人脸...")

                        if(isHelmet):
                            helmetStartEvent.set()
                            in_1 = inputHelmetQueue(frame)
                            helmetQueue.put(in_1)
                            print("开始检测安全帽")
                            ...
                            if self.isFaceTrackerEnabled:
                                ...
                                for fid in fidsToDelete:
                                    ...
                                    if self.isFaceRecognizerEnabled:
                                        ...
                                        cv2.rectangle(realTimeFrame, (x, y), (x + w, y + h), (232, 138, 30), 2)
                                        # face_id, confidence = recognizer.predict(gray[_y:_y + _h, _x:_x + _w])
                                        logging.debug('face_id：{}，confidence：{}'.format(fResult.face_id, fResult.confidence))

                                        if self.isDebugMode:
                                            CoreUI.logQueue.put(
                                                'Debug -> face_id：{}，confidence：{}'.format(fResult.face_id, fResult.confidence))

                                        # 从数据库中获取识别人脸的身份信息
                                        try:
                                            cursor.execute("SELECT * FROM users WHERE face_id=?", (fResult.face_id,))
                                            ...
                                            logging.error('读取数据库异常，系统无法获取Face ID为{}的身份信息'.format(fResult.face_id))
                                            CoreUI.logQueue.put('Error：读取数据库异常，系统无法获取Face ID为{}的身份信息'.format(fResult.face_id))
                                            en_name = ''

                                        # 若置信度评分小于置信度阈值，认为是可靠识别
                                        if fResult.confidence < self.confidenceThreshold:
                                            isKnown = True
                                            isSuccess = True

                                            cv2.putText(realTimeFrame, en_name, (x - 5, y - 10), cv2.FONT_HERSHEY_SIMPLEX,1,(0, 97, 255), 2)
                                            cv2.putText(realTimeFrame, hResult.status, (x - 5, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 97, 255), 2)
                                            ...
                                            try:
                                                ...
                                                if int(datetime.now().strftime('%H%M%S')) < 155900 and list(cursor.execute(
                                                        "select count(*) from workers where name='{0}'".format(en_name)))[
                                                    0][
                                                    0] == 0:
                                                ...
                                                if int(datetime.now().strftime('%H%M%S')) > 160200 and list(cursor.execute(
                                                    ...
                                                    0] == 1:
                                                    cursor.execute(
                                                        "update workers set out_time = '{0}' where name = '{1}'".format(
                                                            out_time,
                                                            en_name))
                                            ...
                                            cv2.putText(realTimeFrame, 'unknown', (x - 5, y - 10),
                                                        cv2.FONT_HERSHEY_SIMPLEX, 1,
                                                        (0, 0, 255), 2)

                                            # 若置信度评分超出自动报警阈值，触发报警信号
                                            if fResult.confidence > self.autoAlarmThreshold:
                                                # 检测报警系统是否开启
...
class inputFaceQueue:
    def __init__(self,x,y,w,h,frame):
        ...
        self.h = h
        self.frame = frame
        self.gray = self.frame[self.y:self.y + self.h, self.x:self.x + self.w]

class inputHelmetQueue:
...
class FaceResult:
    def __init__(self,isSuccess, objectName,face_id, confidence):
        self.isSuccess = isSuccess
        self.obhectName = objectName
        self.face_id = face_id
        self.confidence = confidence

    def setResult(self,isSuccess, oName, faceid, confidence):
        self.isSuccess = isSuccess
        self.oName =  oName
        self.faceid = faceid
        self.confidence = confidence
    ...
    def getResult(self):
        return self.isSuccess,self.obhectName,self.face_id,self.confidence

class HelmetResult:
    def __init__(self,hasHelmet,status):
        self.hasHelmet = hasHelmet
        self.status = status
        # self.x = x
        # self.y = y

    def setResult(self,hasHelmet,status):
        self.hasHelmet = hasHelmet
        self.status = status
        # self.x = x
        # self.y = y
    ...
    def getResult(self):
        return self.hasHelmet,self.status
